...
const examples = getExamples('internal-service-footer');

const componentName = 'footer';

describe('Internal service Footer', () => {
  it('should display first item of meta list', async () => {
    const output = renderExample('internal-service-footer', componentName, examples.meta);
    await runA11yTests(output);
    expect(output).to.have.string('Item 1');
  });

  it('should display both meta and colum lists', async () => {
    const output = renderExample('internal-service-footer', componentName, examples.default_plus_meta);
    await runA11yTests(output);
  ...
  it('should display custom meta (text)', async () => {
    const output = renderExample('internal-service-footer', componentName, examples.custom_meta);
    await runA11yTests(output);
    expect(output).to.have.string('GOV.UK Prototype Kit v7.0.1');
  });

  it('should display meta links and custom meta content', async () => {
    const output = renderExample('internal-service-footer', componentName, examples.meta_links_plus_content);
    await runA11yTests(output);
